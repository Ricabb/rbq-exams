// ===== RBQ Exams – script.js (FR/EN, Exam & Learn, timer, save) =====
let questions = [], currentIndex = 0, answers = {}, timeLeft, examNumber = "1",
    lang = "en", mode = "exam", moduleKey = "admin", timer = null;
...
const i18n = {
  en:{title:"RBQ Exam",back:"← All Exams",prev:"Previous",next:"Next",submit:"Submit",
      ...
      enterName:"Enter a name:",savedTitle:"Saved Exams",open:"Open",del:"Delete",none:"No saved exams yet.",
      learnBanner:"Learn mode: explanations, key terms, and examples are shown.",
      showExp:"Show explanation",hideExp:"Hide explanation",keywords:"Key terms",example:"Example",
  ...
  fr:{title:"Examen RBQ",back:"← Tous les examens",prev:"Précédent",next:"Suivant",submit:"Soumettre",
      ...
      enterName:"Entrez un nom :",savedTitle:"Examens enregistrés",open:"Ouvrir",del:"Supprimer",none:"Aucun examen enregistré.",
      learnBanner:"Mode Apprendre : explications, mots clés et exemples.",
      showExp:"Afficher l’explication",hideExp:"Masquer l’explication",keywords:"Mots clés",example:"Exemple",
...
// ---- Learn mode minimal glossary ----
const glossary=[
  {key:"RBQ",aliases:["rbq","régie du bâtiment"],en:{def:"Regulator for building safety & construction.",ex:"RBQ may suspend a licence for non-compliance."},fr:{def:"Régulateur de la sécurité des bâtiments et de la construction.",ex:"La RBQ peut suspendre une licence en cas de non-conformité."}},
  {key:"BSDQ",aliases:["bsdq"],en:{def:"System for depositing certain trade bids.",ex:"Trades file bids via BSDQ."},fr:{def:"Système de dépôt de certaines soumissions de métiers.",ex:"Les métiers déposent via le BSDQ."}},
  {key:"Lockout",aliases:["cadenassage","lockout","tagout"],en:{def:"Energy isolation procedure.",ex:"Prevents accidental startup."},fr:{def:"Isolement des énergies.",ex:"Empêche les démarrages involontaires."}},
  {key:"Firestopping",aliases:["coupe-feu","firestopping"],en:{def:"Maintains fire rating at penetrations.",ex:"Use a rated system around pipes."},fr:{def:"Maintient le degré coupe-feu.",ex:"Système homologué autour des tuyaux."}},
  {key:"Critical path",aliases:["chemin critique","critical path"],en:{def:"Longest dependent path sets duration.",ex:"Delay here delays project."},fr:{def:"Plus longue chaîne déterminant la durée.",ex:"Retard ici = retard projet."}}
];
...
function explanationBlock(q){
  ...
  const why=(lang==='fr'?`Ici, « ${correct} » reflète la pratique courante pour ${i18n[lang].modules[moduleKey]}.`:`Here “${correct}” reflects common practice for ${i18n[lang].modules[moduleKey]}.`);
...
function saveProgress(){
  localStorage.setItem(`rbq_exam_${lang}_${moduleKey}_${examNumber}_${mode}_answers`,JSON.stringify(answers));
  localStorage.setItem(`rbq_exam_${lang}_${moduleKey}_${examNumber}_${mode}_index`,String(currentIndex));
  localStorage.setItem(`rbq_exam_${lang}_${moduleKey}_${examNumber}_${mode}_timeleft`,String(timeLeft));
}
function loadProgress(){
  answers=JSON.parse(localStorage.getItem(`rbq_exam_${lang}_${moduleKey}_${examNumber}_${mode}_answers`)||"{}");
  const idx=localStorage.getItem(`rbq_exam_${lang}_${moduleKey}_${examNumber}_${mode}_index`);
  const t=localStorage.getItem(`rbq_exam_${lang}_${moduleKey}_${examNumber}_${mode}_timeleft`);
...
function startTimer(){
  clearInterval(timer);
  if(mode==='learn'){ qs('#timer').textContent='—'; return; }
  if(typeof timeLeft!=='number' || isNaN(timeLeft)) timeLeft = 120*60;
...
function render(){
  ...
  qs('#prevBtn').onclick=()=>{ if(currentIndex>0){ currentIndex--; render(); saveProgress(); } };
  qs('#nextBtn').onclick=()=>{ if(currentIndex<questions.length-1){ currentIndex++; render(); saveProgress(); } };
  qs('#submitBtn').onclick=submitExam;
}
function submitExam(){
  const T=i18n[lang]; let score=0;
...
(function init(){
  ...
  qs('#exam-container').innerHTML = '<p style="opacity:.6">Chargement… / Loading…</p>';
  ...
  fetch(file).then(r=>{
    ...
    document.addEventListener('keydown',e=>{if(e.key==='ArrowRight')currentIndex<questions.length-1&& (currentIndex++,render(),saveProgress()); if(e.key==='ArrowLeft')currentIndex>0&&(currentIndex--,render(),saveProgress());});
  }).catch(err=>{
